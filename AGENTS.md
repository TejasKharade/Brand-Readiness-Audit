...
1. Open [`skills/audit-orchestrator/SKILL.md`](skills/audit-orchestrator/SKILL.md)
   — the entrypoint skill — and execute it **literally, step by step, right
...
4. **The task is only complete once a JSON file conforming to
   ...
   has actually been written to disk** — via
   `python skills/audit-orchestrator/scripts/synthesize_report.py --input payload.json --out report.json`
   — and shown to the user. A summary, a plan, or partial findings typed into
...
`python skills/audit-orchestrator/scripts/verify_contracts.py` plus the
project's test suite before considering it done. Planning is appropriate here.
