...
1. **`audit-orchestrator`** *(Entrypoint Master Skill)*: Coordinates the sequential execution of all 5 specialized sub-skills and synthesizes their raw findings into the final report. Each of the 5 `category_scores` is an independent, self-contained number (starts at 100, each finding in that category deducts a fixed amount by severity — `critical` -20, `high` -10, `medium` -5, `low` -2 — scaled by that finding's **confidence**, so a heuristic-only or low-confidence finding counts for less than a script-confirmed fact). There is deliberately no cross-category weighting, no diminishing-returns decay, no foundational gates, and no single blended overall score — the handout's required report schema never asks for one, and a category's score can be verified by hand directly from its findings list.
...
`confidence` (0–1, default 1.0) appears on every finding: how sure the check is the finding is real, as distinct from `severity` (how bad it would be if true). It scales how much that finding deducts from its category's score.

### Scoring: five plain numbers, not one formula

`category_scores` is five independent, self-contained numbers (0-100), one per audit category. Each starts at 100 and is reduced by a fixed amount per finding in that category — `critical` -20, `high` -10, `medium` -5, `low` -2 — scaled by the finding's `confidence`. That's the whole model: no cross-category weighting, no diminishing-returns decay, no gates, and deliberately **no single blended overall score**. The handout's required schema asks only for `site`/`audited_at`/a severity-count `summary`/`findings` — an aggregate score was never required, and a category's number here can be checked by hand straight from the findings list in one pass, rather than through a weighted mean, a decay curve, and a confidence-softened cap. A `crawl_access` or `crawl_render` critical finding still costs its own category the full `-20` like any other critical finding — the "a foundational failure matters most" reasoning is still visible directly in that category's score, it just no longer reaches out to suppress a number that no longer exists.
